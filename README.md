...
ある人が、天気予報を見る前に傘を持っていこうと考える確率を $P_0$ とする。

次に、その人が天気予報から降水確率 $P_R$ という情報を受け取る場合を考える。

本課題ではモデルを単純化するため、

> **降水確率 ＝ 情報を完全に信頼した場合に傘を持つ確率**
...
そこで、受け手が情報をどの程度信頼しているのかを表す「信頼度」 $r$ を導入する。
...
$$
P_0=0.2,\qquad P_R=0.8,\qquad r=0.5
$$
...
Pythonによる実験では、情報への信頼度と降水確率をそれぞれ変化させ、情報の影響度との関係を調べた。
...
プログラムを実行すると、2つの実験が行われ、以下の画像ファイルが生成される。

- `実験結果１.png`
- `実験結果２.png`

生成された画像ファイルは `main.py` と同じディレクトリに保存される。
